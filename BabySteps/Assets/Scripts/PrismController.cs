...
public class PrismController : MonoBehaviour
{
    ...
    private MouseProjection mouseProj;  // script from which to grab data
    // data to be grabbed from MouseProjection script
    ...
    void Update()
    {
        ...
        if (triggered)
        {
            foreach (GameObject prism in prisms)
            {
                ...
                float rStar = Mathf.Sqrt(rSq) - v * timeOffset;
